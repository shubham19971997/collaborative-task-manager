...
export async function createWorkspace(userId: string, body: any) {
  const slug = body.name;

  const existing = await isWorkspaceExist(slug);
  if (existing) {
    throw new Error(
      `The slug "${slug}" is already taken. Try a different name or provide a custom slug.`
    );
  }
...
export const inviteMember = async (workspaceId: string, body: any) => {
  ...
  if (!invitee) {
    throw new Error(
      `No account found with email ${body.email}. They need to register first.`
    );
  }
  ...
  if (existingMember) {
    throw Error(`${invitee.name} is already a member of this workspace`);
  }
...
export const updateWorkspaceMember = async (
  ...
  if (body.role === "OWNER") {
    throw new Error(
      "Cannot assign OWNER role. Use the transfer ownership endpoint instead"
    );
  }
